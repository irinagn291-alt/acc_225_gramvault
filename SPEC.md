# GramVault — Build Specification

> Batch 21AUG, app 03 of 30. This document is the complete brief for
...
**One-line positioning:** Every gram, sealed offline.

| Field | Value |
| --- | --- |
| Product name | GramVault |
| Bundle identifier | `com.gramvault.vault` |
| Domain | https://gramvault.pro |
| Contact URL | https://gramvault.pro/contact-us |
| Deployment target | iOS 17.0 |
| Swift version | 6.2, strict concurrency `complete` |
| Devices | iPhone only, portrait |
| Interface style | Dark |
| Asset prefix | `gvt_` |
| User-Agent | `GramVault/1.0 (iOS; +https://gramvault.pro)` |
...
7. **The app must build clean** with
   `xcodegen generate && xcodebuild -scheme GramVault -destination 'generic/platform=iOS' build`.
...
## 3. Uniqueness assignment for GramVault
...
| --- | --- |
| Architecture | **VIPER** |
| UI approach | **Programmatic UIKit (NSLayoutConstraint anchors)** |
| Naming convention | **Suffix-based protocol naming** |
| File organization | **By layer** |
| Dependency strategy | **SPM remote x1 — SnapKit** |
| Design direction | **Vault / industrial brushed steel** |
| Typography | **DIN Condensed** |
| Navigation pattern | **Single root screen + modal presentations** |
| AI art style | **Photorealistic material texture** |
| Functional twist | **Offline-first** |
| Persistence | **Core Data (NSPersistentContainer)** |
| Screen composition | see 3.6 |
| Scanner technology | **Vision VNDetectBarcodesRequest on the live video buffer** |
| Search endpoint | **/cgi/search.pl, page_size 24** |
| Day key representation | **Int epoch seconds at startOfDay** |

### 3.1 Architecture contract

Strict VIPER per module: View (UIViewController), Interactor (business rules),
Presenter (formatting + orchestration), Entity (plain structs), Router
(navigation). Communication only through protocols; Presenter never imports
UIKit types beyond what protocols expose. Each module has a `Module` assembler
that wires the five pieces.
...
No storyboards, no XIBs. Every view built in code, laid out with
`NSLayoutConstraint` anchor APIs. `UIWindow` created in `SceneDelegate`.
Set `INFOPLIST_KEY_UIApplicationSceneManifest_Generation: NO` and supply a
manual scene manifest.

### 3.3 Naming contract

Convention: Suffix-based protocol naming.

Examples to follow: `VaultViewProtocol`, `VaultInteractorInput`, `VaultPresenterOutput`, `VaultRouterProtocol`

Apply it to types, files, properties, methods and asset names alike.

### 3.4 Dependency contract

SnapKit only for constraint DSL in the heavier composite views; simple views use raw anchors.

### 3.5 Navigation contract

One vault screen; everything else is `present(_:animated:)` as a modal. No push navigation at all.
...
One vault screen; everything else is modal. Search and Scan share one modal
with a segmented switch. Detail into Assign is a two-page modal wizard.
Physical screens: Vault, SourceModal, AssignWizard, Targets, Wish.
...
Technology: **Vision VNDetectBarcodesRequest on the live video buffer**

Machined bracket overlay; a solid lock indicator when a code holds.
...
- Open Food Facts search uses **/cgi/search.pl, page_size 24**.
- A day is represented internally as **Int epoch seconds at startOfDay**. Use this consistently in
  storage, in queries and in identifiers.
- The demo seed flag key is `gvt.demo.v1`.
- `NSCameraUsageDescription` is exactly: "GramVault reads barcodes to seal products into your local vault."
...
```
GramVault/
  Presentation/ Domain/ Data/ Common/
  Assets.xcassets/
```
...
### 5.6 Assign
Choose a slot — Vault A, Vault B, Vault C, Loose Change — and choose
eaten-today or a future date. Loose Change is eaten-only and must be unavailable
...
https://gramvault.pro/contact-us.
...
Direction: **Vault / industrial brushed steel**

### 7.1 Palette

| Token | Hex | Use |
| --- | --- | --- |
| `background` | `#2C3338` | Screen background |
| `surface` | `#373E44` | Cards, rows, sheets |
| `ink` | `#E8ECEF` | Primary text and icons |
| `accent` | `#FFB300` | Primary action, key figure, progress fill |
| `muted` | `#8A939A` | Secondary text, dividers, disabled |
...
`DINCondensed-Bold` for headers and numerals, system for body when DIN is unavailable.
...
Chosen technology: **Core Data (NSPersistentContainer)**

Programmatic `NSManagedObjectModel` (no .xcdatamodeld). Entities: VaultEntry, VaultTarget, VaultProduct.
...
Set `User-Agent: GramVault/1.0 (iOS; +https://gramvault.pro)` on every request. Never reuse another app's string.
Use the **/cgi/search.pl, page_size 24** search endpoint for this app.
...
INFOPLIST_KEY_NSCameraUsageDescription: GramVault reads barcodes to seal products into your local vault.
...
## 12. Functional twist: Offline-first

Full local catalog: every product fetched from OFF is permanently cached and
searchable offline. Network is strictly optional — a 'Sealed' toggle disables
it entirely and the app stays fully usable on the cached catalog.
...
Art style: **Photorealistic material texture**
...
given. Every name carries the `gvt_` prefix. No asset may be reused
...
```bash
sips -g pixelWidth -g pixelHeight -g hasAlpha gvt_AppIcon.png
...
```bash
sips -s format jpeg gvt_AppIcon.png --out tmp.jpg \
  && sips -s format png tmp.jpg --out gvt_AppIcon.png && rm tmp.jpg
```
...
| --- | --- | --- | --- | --- |
| 1 | `gvt_AppIcon` | 1024x1024 | **NO** | App Store icon. NO alpha channel, NO transparency, NO text, NO rounded corners, NO drop shadow outside the canvas. |
| 2 | `gvt_Splash` | 1290x2796 | allowed | Launch background. The middle third must stay quiet so the wordmark reads on top. |
| 3 | `gvt_Onboarding1` | 1024x1536 | allowed | Onboarding page 1 illustration: what the app is for. |
| 4 | `gvt_Onboarding2` | 1024x1536 | allowed | Onboarding page 2 illustration: scan and search. |
| 5 | `gvt_Onboarding3` | 1024x1536 | allowed | Onboarding page 3 illustration: setting daily targets. |
| 6 | `gvt_EmptyLog` | 1024x1024 | allowed | Empty state: nothing logged today yet. Calm and inviting, never sad. |
| 7 | `gvt_EmptySearch` | 1024x1024 | allowed | Empty state: search returned no products. |
| 8 | `gvt_EmptyPlan` | 1024x1024 | allowed | Empty state: no planned entries. |
| 9 | `gvt_EmptyWish` | 1024x1024 | allowed | Empty state: wish list has no items. |
| 10 | `gvt_SlotVaultA` | 512x512 | allowed | Icon for the 'Vault A' slot. |
| 11 | `gvt_SlotVaultB` | 512x512 | allowed | Icon for the 'Vault B' slot. |
| 12 | `gvt_SlotVaultC` | 512x512 | allowed | Icon for the 'Vault C' slot. |
| 13 | `gvt_SlotLooseChange` | 512x512 | allowed | Icon for the 'Loose Change' slot. |
| 14 | `gvt_MacroProtein` | 512x512 | allowed | Macro icon: protein. Must be visually distinct from carbs and fat at 24pt. |
| 15 | `gvt_MacroCarbs` | 512x512 | allowed | Macro icon: carbohydrates. |
| 16 | `gvt_MacroFat` | 512x512 | allowed | Macro icon: fat. |
| 17 | `gvt_ProductPlaceholder` | 600x600 | allowed | Fallback thumbnail shown when a product has no image. Used in every list row. |
| 18 | `gvt_CardBackdrop` | 1200x800 | allowed | Backdrop art for the product detail card. Low contrast so text stays readable. |
| 19 | `gvt_Texture` | 2048x2048 | allowed | Tiling background texture used at low opacity behind content. MUST tile with no visible seam. |
| 20 | `gvt_ControlFace` | 512x512 | allowed | Custom control artwork used for the primary interactive element. |
| 21 | `gvt_ScanOverlay` | 1024x1024 | required | Camera overlay art framing the barcode. The centre must be fully transparent. |
| 22 | `gvt_TwistHero` | 1024x1024 | allowed | Hero art for the 'Offline-first' feature screen. |
| 23 | `gvt_SuccessMark` | 512x512 | allowed | Shown briefly when a food is logged successfully. |
| 24 | `gvt_HeaderDecor` | 1200x600 | allowed | Decorative header accent on the main screen. |

### Prompt per asset

**`gvt_AppIcon`** — 1024x1024

```
4k material texture, brushed stainless steel, realistic, octane render, industrial machined surface, amber warning light reflection, the app's single emblem, centred, filling the canvas edge to edge
```

**`gvt_Splash`** — 1290x2796

```
4k material texture, brushed stainless steel, realistic, octane render, industrial machined surface, amber warning light reflection, a vertical hero composition with a calm, uncluttered centre band
```

**`gvt_Onboarding1`** — 1024x1536

```
4k material texture, brushed stainless steel, realistic, octane render, industrial machined surface, amber warning light reflection, a person or object representing discovering what is in packaged food
```

**`gvt_Onboarding2`** — 1024x1536

```
4k material texture, brushed stainless steel, realistic, octane render, industrial machined surface, amber warning light reflection, a scanning or measuring motif showing a product being identified
```

**`gvt_Onboarding3`** — 1024x1536

```
4k material texture, brushed stainless steel, realistic, octane render, industrial machined surface, amber warning light reflection, a goal or target motif showing daily progress being met
```

**`gvt_EmptyLog`** — 1024x1024

```
4k material texture, brushed stainless steel, realistic, octane render, industrial machined surface, amber warning light reflection, an empty vessel, surface or container waiting to be filled
```

**`gvt_EmptySearch`** — 1024x1024

```
4k material texture, brushed stainless steel, realistic, octane render, industrial machined surface, amber warning light reflection, a search motif that has come back with nothing found
```

**`gvt_EmptyPlan`** — 1024x1024

```
4k material texture, brushed stainless steel, realistic, octane render, industrial machined surface, amber warning light reflection, an empty schedule, grid or horizon with nothing scheduled
```

**`gvt_EmptyWish`** — 1024x1024

```
4k material texture, brushed stainless steel, realistic, octane render, industrial machined surface, amber warning light reflection, an empty basket, list or shelf
```

**`gvt_SlotVaultA`** — 512x512

```
4k material texture, brushed stainless steel, realistic, octane render, industrial machined surface, amber warning light reflection, a morning motif appropriate to the theme
```

**`gvt_SlotVaultB`** — 512x512

```
4k material texture, brushed stainless steel, realistic, octane render, industrial machined surface, amber warning light reflection, a midday motif appropriate to the theme
```

**`gvt_SlotVaultC`** — 512x512

```
4k material texture, brushed stainless steel, realistic, octane render, industrial machined surface, amber warning light reflection, an evening motif appropriate to the theme
```

**`gvt_SlotLooseChange`** — 512x512

```
4k material texture, brushed stainless steel, realistic, octane render, industrial machined surface, amber warning light reflection, a small extra or in-between motif appropriate to the theme
```

**`gvt_MacroProtein`** — 512x512

```
4k material texture, brushed stainless steel, realistic, octane render, industrial machined surface, amber warning light reflection, a symbol standing for protein, rendered as a single clear emblem
```

**`gvt_MacroCarbs`** — 512x512

```
4k material texture, brushed stainless steel, realistic, octane render, industrial machined surface, amber warning light reflection, a symbol standing for carbohydrate, rendered as a single clear emblem
```

**`gvt_MacroFat`** — 512x512

```
4k material texture, brushed stainless steel, realistic, octane render, industrial machined surface, amber warning light reflection, a symbol standing for dietary fat, rendered as a single clear emblem
```

**`gvt_ProductPlaceholder`** — 600x600

```
4k material texture, brushed stainless steel, realistic, octane render, industrial machined surface, amber warning light reflection, a generic packaged grocery item with no readable branding
```

**`gvt_CardBackdrop`** — 1200x800

```
4k material texture, brushed stainless steel, realistic, octane render, industrial machined surface, amber warning light reflection, an abstract backdrop suitable for sitting behind a product card
```

**`gvt_Texture`** — 2048x2048

```
4k material texture, brushed stainless steel, realistic, octane render, industrial machined surface, amber warning light reflection, a seamless repeating surface pattern
```

**`gvt_ControlFace`** — 512x512

```
4k material texture, brushed stainless steel, realistic, octane render, industrial machined surface, amber warning light reflection, the face of a single physical control such as a dial, key or slider handle
```

**`gvt_ScanOverlay`** — 1024x1024

```
4k material texture, brushed stainless steel, realistic, octane render, industrial machined surface, amber warning light reflection, a framing reticle or targeting bracket, open in the middle
```

**`gvt_TwistHero`** — 1024x1024

```
4k material texture, brushed stainless steel, realistic, octane render, industrial machined surface, amber warning light reflection, an emblem representing this app's signature feature
```

**`gvt_SuccessMark`** — 512x512

```
4k material texture, brushed stainless steel, realistic, octane render, industrial machined surface, amber warning light reflection, a confirmation mark or celebratory emblem
```

**`gvt_HeaderDecor`** — 1200x600

```
4k material texture, brushed stainless steel, realistic, octane render, industrial machined surface, amber warning light reflection, a wide decorative band or ornament
```
...
- `gvt_Texture` must tile seamlessly. Verify by placing four copies
  edge to edge.
- `gvt_ScanOverlay` needs a genuinely transparent centre.
...
| --- | --- | --- | --- | --- | --- |
| Butter | `3178530403022` | 717 | 0.9 | 0.1 | 81.1 |
| Oat Milk | `7394376616037` | 45 | 1.0 | 6.7 | 1.5 |
| Soy Sauce | `0041390001017` | 53 | 8.1 | 4.9 | 0.6 |
| Greek Yogurt Plain | `5060335637000` | 59 | 10.0 | 3.6 | 0.4 |
| Canned Chickpeas | `8410054000129` | 119 | 7.1 | 16.2 | 2.6 |
| Espresso Coffee | `8000070025431` | 2 | 0.1 | 0.0 | 0.0 |
...
| Role | Label in GramVault | Planned | Eaten |
| --- | --- | --- | --- |
| Morning meal | Vault A | yes | yes |
| Midday meal | Vault B | yes | yes |
| Evening meal | Vault C | yes | yes |
| Snack | Loose Change | no | yes |
...
Add a unit test target `GramVaultTests` covering at minimum:
...
- [ ] `xcodebuild -scheme GramVault -destination 'generic/platform=iOS' build` succeeds.
...
- [ ] All four slots accept entries; Loose Change rejects future dates.
...
**Uniqueness**
- [ ] Architecture matches **VIPER** with no leakage across layers.
- [ ] UI approach matches **Programmatic UIKit (NSLayoutConstraint anchors)**.
- [ ] Navigation matches **Single root screen + modal presentations**.
- [ ] Screen composition follows section 3.6, not the generic list in section 5.
- [ ] Scanner uses **Vision VNDetectBarcodesRequest on the live video buffer**.
- [ ] Day key is **Int epoch seconds at startOfDay**.
...
- [ ] Demo seed runs only on Simulator, once, behind `gvt.demo.v1`.
...
**Assets**
- [ ] All 24 images generated in the **Photorealistic material texture** style, prefixed `gvt_`.
